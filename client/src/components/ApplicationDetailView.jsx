  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <button
          onClick={onBack}
          className="flex items-center gap-2 text-slate-400 hover:text-slate-100 text-sm font-semibold transition-colors bg-[#14192a]/60 px-3.5 py-2 rounded-xl border border-slate-700/60"
        >
        ...
        <div className="flex flex-wrap items-center gap-2 bg-[#14192a]/80 p-2 rounded-2xl border border-slate-700/60">
      ...
      {/* Main Candidate Card */}
      <div className="bg-[#14192a]/80 border border-slate-700/60 rounded-3xl p-6 sm:p-8 shadow-2xl flex flex-col gap-8 backdrop-blur-xl">
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="flex items-center gap-3 flex-wrap">
                <h1 className="text-2xl font-extrabold text-slate-100">{candidate?.fullName || 'Ứng viên'}</h1>
              ...
              <p className="text-xs text-orange-400 font-semibold mt-2 flex items-center gap-1">
                ...
                Vị trí ứng tuyển: <strong className="text-slate-100 ml-1">{job?.title}</strong> ({job?.department})
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col gap-5">
            <div className="bg-[#0d1117]/70 p-5 rounded-2xl border border-slate-700/60/80 flex flex-col gap-3">
          ...
          <div className="bg-[#0d1117]/70 p-5 rounded-2xl border border-slate-700/60/80 flex flex-col justify-between gap-4">
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                {candidate?.cvPath && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-xs text-orange-400 hover:text-slate-100 font-bold flex items-center gap-1 bg-orange-500/10 hover:bg-orange-500 px-3 py-1 rounded-xl border border-orange-500/20 transition-all"
                  >
              ...
              <div className="bg-[#14192a] p-4 rounded-xl text-xs text-slate-300 font-mono overflow-y-auto max-h-48 border border-slate-700/60/80 whitespace-pre-wrap leading-relaxed">
        ...
        <div className="border-t border-slate-700/60/80 pt-6 flex flex-col gap-4">
          <h3 className="text-sm font-bold text-slate-100 uppercase tracking-wider text-orange-400 flex items-center gap-2">
            <HelpCircle className="w-4.5 h-4.5 text-orange-400" />
            Bộ Câu Hỏi Phỏng Vấn Gợi Ý Bởi AI (Suggested Interview Questions)
          </h3>

          {questions.length === 0 ? (
            <p className="text-xs text-slate-400 bg-[#0d1117]/50 p-4 rounded-xl border border-slate-700/60">
              Chưa tạo câu hỏi phỏng vấn.
            </p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {questions.map((q, idx) => (
                <div key={q.id || idx} className="bg-[#0d1117]/70 p-4 rounded-2xl border border-slate-700/60/80 flex items-start gap-3 hover:border-orange-500/30 transition-all">
